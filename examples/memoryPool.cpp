...
// The resource manger is divided into two parts, the base memory manager and the block memory manager.
//
// The base memory manager contains all sTypes (structure types e.g. LvnBuffer, LvnTexture) allocated into one large memory chunk,
// each sType has a size in bytes and a count parameter that specifies how many objects are can be allocated, by default levikno assumes a number
// of each sType will be used depending on the functionality of the object (for example, LvnWindow objects will typically be used/allocated less than LvnBuffers).
//
...
//
// A memory block is divided into smaller sections for each sType, a LvnMemoryBinding manages the number of objects
// allocated for a sType with the section it is given in the memory block
//
// When a memory binding is full, meaning all allocations within the binding have been used, a second memory block and binding is created that is linked
// at the end of the first binding. Note that this is similar to how a linked list works with each node being its own memory block.
//
// Note that further memory blocks created after the base memory block will be seperate for each sType, meaning a memory block
// can be allocated for only one structure type such as LvnWindow, LvnBuffer, etc.
//
// When an object is destroyed, the memory index of that object will be returned back to the memory binding, the same memory index can be used again when
...
//                  v                                            v
//           +---------------------------------------+------------------------+---------+
//  memory:  | X  |    |    |    |    |    |    |    | X  | X  |    |    |    | X  | X  |   <---- Ex. Logger LvnBinding is full, levikno allocates a second memory block/binding
...
//                     |                             |                       |                the base memory block where every sType binding is contained within one allocation
//  Bindings:       Buffers                       Textures                Loggers
//                     |                             |                       |
//            /-----------------\        /----------------------\        /-------\
//                                                                             v  <---- next memory index to be used when creating new Logger object
//           +-------------------+      +------------------------+      +---------+
//  memory:  |    |    |    |    |      |    |    |    |    |    |      | X  |    |
//           +-------------------+      +-------------------+----+      +---------+
...
int main(int argc, char** argv)
{
    ...
    //       where l1 was originally allocated


    // it is still recomended to call the destroy functions for each object before
