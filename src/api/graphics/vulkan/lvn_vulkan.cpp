...
LvnResult vksImplCreateShaderFromSrc(LvnShader* shader, const LvnShaderCreateInfo* createInfo)
{
    ...
    return Lvn_Result_Success;
#else
    LVN_CORE_ERROR("[vulkan]: cannot create shader, glslang is not included, unable to compile shader source code");
    LVN_CORE_INFO("[vulkan]: if glslang is unavailable, shader code must be compiled externaly and loaded as a binary");
    return Lvn_Result_Failure;
#endif
}
