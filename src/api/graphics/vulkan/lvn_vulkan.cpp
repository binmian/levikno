...
LvnResult vksImplCreateShaderFromSrc(LvnShader* shader, LvnShaderCreateInfo* createInfo)
{
	...
	if (vks::compileShaderToSPIRV(GLSLANG_STAGE_VERTEX, createInfo->vertexSrc, vertData) == Lvn_Result_Failure)
	{
		LVN_CORE_ERROR("[vulkan] failed to create vertex shader module for shader at (%p)\nsrc code:\n%s", shader, createInfo->vertexSrc);
		return Lvn_Result_Failure;
	}

	if (vks::compileShaderToSPIRV(GLSLANG_STAGE_FRAGMENT, createInfo->fragmentSrc, fragData) == Lvn_Result_Failure)
	{
...
LvnResult vksImplCreateFrameBuffer(LvnFrameBuffer* frameBuffer, LvnFrameBufferCreateInfo* createInfo)
{
	...
	frameBufferData->colorAttachments = LvnVector<LvnFrameBufferColorAttachment>(createInfo->pColorAttachments, createInfo->colorAttachmentCount);
