...
LvnResult implGlsCreateContext(LvnGraphicsContext* graphicsContext)
{
    ...
    s_OglBackends->defaultOglPipelineFixedFuncs = lvn::configPipelineFixedFuncInit();
    ...
    graphicsContext->getPhysicalDevices = implGlsGetPhysicalDevices;
    graphicsContext->checkPhysicalDeviceSupport = implGlsCheckPhysicalDeviceSupport;
    graphicsContext->setPhysicalDevice = implGlsSetPhysicalDevice;
    graphicsContext->createShaderFromSrc = implGlsCreateShaderFromSrc;
    graphicsContext->createShaderFromFileSrc = implGlsCreateShaderFromFileSrc;
    graphicsContext->createShaderFromFileBin = implGlsCreateShaderFromFileBin;
    graphicsContext->createDescriptorLayout = implGlsCreateDescriptorLayout;
    graphicsContext->createPipeline = implGlsCreatePipeline;
    graphicsContext->createFrameBuffer = implGlsCreateFrameBuffer;
    graphicsContext->createBuffer = implGlsCreateBuffer;
    graphicsContext->createSampler = implGlsCreateSampler;
    graphicsContext->createTexture = implGlsCreateTexture;
    graphicsContext->createCubemap = implGlsCreateCubemap;

    graphicsContext->destroyShader = implGlsDestroyShader;
    graphicsContext->destroyDescriptorLayout = implGlsDestroyDescriptorLayout;
    graphicsContext->destroyPipeline = implGlsDestroyPipeline;
    graphicsContext->destroyFrameBuffer = implGlsDestroyFrameBuffer;
    graphicsContext->destroyBuffer = implGlsDestroyBuffer;
    graphicsContext->destroySampler = implGlsDestroySampler;
    graphicsContext->destroyTexture = implGlsDestroyTexture;
    graphicsContext->destroyCubemap = implGlsDestroyCubemap;

    graphicsContext->renderBeginNextFrame = implGlsRenderBeginNextFrame;
    graphicsContext->renderDrawSubmit = implGlsRenderDrawSubmit;
    graphicsContext->renderBeginCommandRecording = implGlsRenderBeginCommandRecording;
    graphicsContext->renderEndCommandRecording = implGlsRenderEndCommandRecording;

    if (lvnctx->multithreading)
    {
        graphicsContext->renderCmdDraw = implGlsRecordCmdDraw;
        graphicsContext->renderCmdDrawIndexed = implGlsRecordCmdDrawIndexed;
        graphicsContext->renderCmdDrawInstanced = implGlsRecordCmdDrawInstanced;
        graphicsContext->renderCmdDrawIndexedInstanced = implGlsRecordCmdDrawIndexedInstanced;
        graphicsContext->renderCmdSetStencilReference = implGlsRecordCmdSetStencilReference;
        graphicsContext->renderCmdSetStencilMask = implGlsRecordCmdSetStencilMask;
        graphicsContext->renderCmdBeginRenderPass = implGlsRecordCmdBeginRenderPass;
        graphicsContext->renderCmdEndRenderPass = implGlsRecordCmdEndRenderPass;
        graphicsContext->renderCmdBindPipeline = implGlsRecordCmdBindPipeline;
        graphicsContext->renderCmdBindVertexBuffer = implGlsRecordCmdBindVertexBuffer;
        graphicsContext->renderCmdBindIndexBuffer = implGlsRecordCmdBindIndexBuffer;
        graphicsContext->renderCmdBindDescriptorSets = implGlsRecordCmdBindDescriptorSets;
        graphicsContext->renderCmdBeginFrameBuffer = implGlsRecordCmdBeginFrameBuffer;
        graphicsContext->renderCmdEndFrameBuffer = implGlsRecordCmdEndFrameBuffer;
    }
    else
    {
        graphicsContext->renderCmdDraw = implGlsRenderCmdDraw;
        graphicsContext->renderCmdDrawIndexed = implGlsRenderCmdDrawIndexed;
        graphicsContext->renderCmdDrawInstanced = implGlsRenderCmdDrawInstanced;
        graphicsContext->renderCmdDrawIndexedInstanced = implGlsRenderCmdDrawIndexedInstanced;
        graphicsContext->renderCmdSetStencilReference = implGlsRenderCmdSetStencilReference;
        graphicsContext->renderCmdSetStencilMask = implGlsRenderCmdSetStencilMask;
        graphicsContext->renderCmdBeginRenderPass = implGlsRenderCmdBeginRenderPass;
        graphicsContext->renderCmdEndRenderPass = implGlsRenderCmdEndRenderPass;
        graphicsContext->renderCmdBindPipeline = implGlsRenderCmdBindPipeline;
        graphicsContext->renderCmdBindVertexBuffer = implGlsRenderCmdBindVertexBuffer;
        graphicsContext->renderCmdBindIndexBuffer = implGlsRenderCmdBindIndexBuffer;
        graphicsContext->renderCmdBindDescriptorSets = implGlsRenderCmdBindDescriptorSets;
        graphicsContext->renderCmdBeginFrameBuffer = implGlsRenderCmdBeginFrameBuffer;
        graphicsContext->renderCmdEndFrameBuffer = implGlsRenderCmdEndFrameBuffer;
    }

    graphicsContext->bufferUpdateData = implGlsBufferUpdateData;
    graphicsContext->bufferResize = implGlsBufferResize;
    graphicsContext->allocateDescriptorSet = implGlsAllocateDescriptorSet;
    graphicsContext->updateDescriptorSetData = implGlsUpdateDescriptorSetData;
    graphicsContext->frameBufferGetImage = implGlsFrameBufferGetImage;
    graphicsContext->frameBufferGetRenderPass = implGlsFrameBufferGetRenderPass;
    graphicsContext->framebufferResize = implGlsFrameBufferResize;
    graphicsContext->frameBufferSetClearColor = implGlsFrameBufferSetClearColor;
    graphicsContext->findSupportedDepthImageFormat = implGlsFindSupportedDepthImageFormat;
...
LvnResult implGlsCreateTextureSampler(LvnTexture* texture, const LvnTextureSamplerCreateInfo* createInfo)
{
...
LvnResult implGlsCreateCubemapHdr(LvnCubemap* cubemap, const LvnCubemapHdrCreateInfo* createInfo)
{
...
void implGlsFrameBufferResize(LvnFrameBuffer* frameBuffer, uint32_t width, uint32_t height)
{
    OglFramebufferData* frameBufferData = static_cast<OglFramebufferData*>(frameBuffer->frameBufferData);
...
void implGlsFrameBufferSetClearColor(LvnFrameBuffer* frameBuffer, uint32_t attachmentIndex, float r, float g, float b, float a)
{
    glClearColor(r, g, b, a);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}
...
{
    LvnCmdDraw cmd{};
    cmd.header.callFunc = lvn::implGlsDrawBuffCmdDraw;
...
void implGlsRecordCmdDrawIndexed(LvnWindow* window, uint32_t indexCount)
{
    ...
    cmd.header.callFunc = lvn::implGlsDrawBuffCmdDrawIndexed;
...
void implGlsRecordCmdDrawInstanced(LvnWindow* window, uint32_t vertexCount, uint32_t instanceCount, uint32_t firstInstance)
{
    ...
    cmd.header.callFunc = lvn::implGlsDrawBuffCmdDrawInstanced;
...
void implGlsRecordCmdDrawIndexedInstanced(LvnWindow* window, uint32_t indexCount, uint32_t instanceCount, uint32_t firstInstance)
{
    ...
    cmd.header.callFunc = lvn::implGlsDrawBuffCmdDrawIndexedInstanced;
...
void implGlsRecordCmdSetStencilReference(uint32_t reference)
{
    ...
    cmd.header.callFunc = lvn::implGlsDrawBuffCmdSetStencilReference;
...
void implGlsRecordCmdBeginRenderPass(LvnWindow* window, float r, float g, float b, float a)
{
    ...
    cmd.header.callFunc = lvn::implGlsDrawBuffCmdBeginRenderPass;
...
void implGlsRecordCmdEndRenderPass(LvnWindow* window)
{
    ...
    cmd.header.callFunc = lvn::implGlsDrawBuffCmdEndRenderPass;
...
void implGlsRecordCmdBindPipeline(LvnWindow* window, LvnPipeline* pipeline)
{
    ...
    cmd.header.callFunc = lvn::implGlsDrawBuffCmdBindPipeline;
...
void implGlsRecordCmdBindVertexBuffer(LvnWindow* window, uint32_t firstBinding, uint32_t bindingCount, LvnBuffer** pBuffers, uint64_t* pOffsets)
{
    ...
    cmd.header.callFunc = lvn::implGlsDrawBuffCmdBindVertexBuffer;
...
void implGlsRecordCmdBindIndexBuffer(LvnWindow* window, LvnBuffer* buffer, uint64_t offset)
{
    ...
    cmd.header.callFunc = lvn::implGlsDrawBuffCmdBindIndexBuffer;
...
void implGlsRecordCmdBindDescriptorSets(LvnWindow* window, LvnPipeline* pipeline, uint32_t firstSetIndex, uint32_t descriptorSetCount, LvnDescriptorSet** pDescriptorSets)
{
    ...
    cmd.header.callFunc = lvn::implGlsDrawBuffCmdBindDescriptorSets;
...
void implGlsRecordCmdBeginFrameBuffer(LvnWindow* window, LvnFrameBuffer* frameBuffer)
{
    ...
    cmd.header.callFunc = lvn::implGlsDrawBuffCmdBeginFrameBuffer;
...
void implGlsRecordCmdEndFrameBuffer(LvnWindow* window, LvnFrameBuffer* frameBuffer)
{
    ...
    cmd.header.callFunc = lvn::implGlsDrawBuffCmdEndFrameBuffer;
